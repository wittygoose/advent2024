...
void process(int& x, int& y, std::vector<std::vector<int>>& field, int& width, int& rating) {
    ...
    for (auto& dir : directions) {
        ...
        if (next == target) {
            if (next == 9) ++rating;
            else process(new_x, new_y, field, width, rating);
        }
    }
}
...
int main() {
    ...
    for (auto& pair : heads) {
        int rating = 0;
        process(pair.first, pair.second, field, width, rating);
        result += rating;
    }
    file.close();
